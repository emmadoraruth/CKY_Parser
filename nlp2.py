...
def count_hashes(filename):
    file = open(filename, 'r')
    binary_counts = collections.defaultdict(default_factory)
    unary_counts = collections.defaultdict(default_factory)
    start_counts = {}
    abundant = set()
    for line in file: # Parse file by line
        ...
        if len(parsed) > 0:
            if parsed[1] == 'UNARYRULE': # Unary rules
                ...
                unary_counts[parsed[2]][rule] = int(parsed[0])
                ...
                binary_counts[parsed[2]][rule] = int(parsed[0])
            else: # Terminals
...
def cky(filename, counts_filename, predictions_filename):
    ...
    # Generate set of abundant (non-rare) words
    abundant = counts[3]
    # Generate hash of q parameters (ML estimates of rules)
    q_hash = q_rules(binary_counts, unary_counts, start_counts)
    # Genereate set of non-terminals from count hash
            ...
            for i in range(1, n+1):
                for x in non_terminals:
                    if sentence[i-1] in abundant: # Abundant (unrare) terminal word
                        ...
                        rule = x + ' _RARE_'
                    if rule in unary_counts[x]: # Unary rules in grammar
